...
const Navbar = () => {
    ...
    const handleNavigation = (path, isHomeLink) => {
        if (isHomeLink) {
            navigate(path);
            return;
        }
    ...
    return (_jsx(AppBar, { position: "fixed", sx: {
            ...
            borderBottom: "1px solid rgba(90, 16, 143, 0.1)",
        }, children: _jsx(Toolbar, { sx: { justifyContent: "center", gap: 2 }, children: navigationItems.map(({ path, icon: Icon, tooltip, ariaLabel, isHomeLink }) => (_jsx(Tooltip, { title: tooltip, arrow: true, children: _jsx(IconButton, { onClick: () => handleNavigation(path, isHomeLink), "aria-label": ariaLabel, sx: {
                        ...
                        borderRadius: "12px",
                        margin: "0 4px"
                    }, children: _jsx(Icon, { size: 24 }) }) }, path))) }) }));
};
